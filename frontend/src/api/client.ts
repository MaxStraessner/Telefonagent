...
export const api = {
  ...
  finalizeAppointmentBooking: (value: { session_id: string; tool_call_id: string; service_id: string; appointment_type_id: string; customer_name: string; customer_phone: string | null; customer_email: string | null; start_at: string; timezone: string; confirmation_version: number; confirmed: true }) => request<CalendarBookingResult>("/calendar/tools/finalize-appointment-booking", { method: "POST", body: value }),
