...
export function SystemPage() {
  ...
  return <DataPage>{(data) => <div className="page"><PageHeader eyebrow="Technik" title="System" description="Transparenter Status der lokalen Plattformkomponenten." />
    ...
    <section className="card integrations"><h2>Integrationen</h2><div className="integration-row"><div><strong>OpenAI Realtime</strong><span>Sprachverbindung</span></div><StatusBadge active={data.platformStatus.realtime_voice_configured}>{data.platformStatus.realtime_voice_configured ? "Konfiguriert" : "Nicht eingerichtet"}</StatusBadge></div><div className="integration-row"><div><strong>Telefonie</strong><span>Twilio Voice</span></div><StatusBadge active={data.platformStatus.telephony_configured}>{data.platformStatus.telephony_configured ? "Konfiguriert" : "Nicht eingerichtet"}</StatusBadge></div><div className="integration-row"><div><strong>Kalender</strong><span>Externer Provider</span></div><StatusBadge active={data.platformStatus.calendar_configured}>Lokaler Testmodus</StatusBadge></div></section>
