...
def compile_agent_prompt(bundle: AgentBundle, greeting: str) -> str:
    ...
    sections = [
        ...
        _section(SECTION_NAMES[3], [
            ...
            "Sprich neutrales, natürliches Standarddeutsch ohne englischen Akzent und ohne englische Satzmelodie.",
            "Sprich Zahlen, Uhrzeiten, Datumsangaben, Wochentage, Monate, Umlaute und deutsche Namen nach deutschen Ausspracheregeln aus: 11:00 als elf Uhr, 11:30 als elf Uhr dreißig und 23. Juli als dreiundzwanzigster Juli.",
        ...
        _section(SECTION_NAMES[13], ([
            ...
            "Wähle eine angebotene Alternative ausschließlich mit select_booking_slot und ihrer unveränderten slot_id aus.",
            ...
            "Rufe prepare_appointment_confirmation auf. Lies ausschließlich dessen Zusammenfassung vor und hole danach eine ausdrückliche Bestätigung ein.",
            "Rufe finalize_appointment_booking erst nach dieser ausdrücklichen Bestätigung mit der aktuellen confirmation_version auf.",
            "Eine eindeutige Zustimmung ist sinngemäß zu verstehen und nicht an einen festen Satz gebunden. Dazu zählen im passenden Kontext zum Beispiel: Ja, Ja bitte, Genau, Das passt, Mach das, Bitte eintragen, Gerne, Okay, In Ordnung und Den nehme ich.",
            "Frage nach einer eindeutigen Zustimmung nicht ein zweites Mal. Bei Ablehnung buchst du nicht. Bei einem Änderungswunsch aktualisierst du zuerst die Daten, fasst den geänderten Termin zusammen und holst danach eine neue Zustimmung ein. Nur eine tatsächlich unklare Antwort erfordert eine kurze Rückfrage.",
            ...
            "Kündige eine Prüfung nur an, wenn du das erforderliche Werkzeug in derselben Gesprächsrunde tatsächlich aufrufst. Wenn Angaben fehlen, frage stattdessen gezielt danach.",
            "Nach jedem erfolgreichen, leeren oder fehlerhaften Werkzeugergebnis sprichst du ohne neue Benutzereingabe weiter: nenne das Ergebnis oder den Fehler verständlich und stelle genau die nächste notwendige Frage.",
